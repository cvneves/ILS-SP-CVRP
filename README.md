...
In its current state, the code is aimed specifically at the canonical version of the VRP, which considers only the vehicle capacities. Furthermore, slight modifications were made to the previously cited versions in order to address the more recent benchmark set proposed by [[2](#2)]. Some of the changes include algorithm parameters (e.g. number of iterations, route pool acceptance tolerance), as well as attending the new standard of unrestricted vehicles in the solutions.
